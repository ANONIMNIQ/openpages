...
const TopicCard: React.FC<TopicCardProps> = ({
  ...
  return (
    <motion.div
      onClick={onClick}
      className={`cursor-pointer rounded-xl group transition-all ${isTall ? 'h-full flex flex-col pt-2 pb-6 px-6' : 'border-b border-gray-100 py-10 px-4'}`}
    >
      <div className="flex-1">
        {renderBadge(false)}
        <h2 className="text-2xl font-black leading-tight mb-4 group-hover:underline decoration-2 underline-offset-4">
          {title}
        </h2>
